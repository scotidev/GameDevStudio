...
This documents the JSON payloads each Copilot hook receives on stdin for every event type.
...
Fired when a Copilot session begins. **No stdin input** — the hook just runs and its stdout is shown to Copilot as context.
...
Fired when the Copilot session ends. **No stdin input** — the hook runs for cleanup and logging.
...
| 2         | Block (stderr shown to Copilot) | PreToolUse only   |
