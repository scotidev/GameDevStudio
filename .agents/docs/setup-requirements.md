...
- Hooks use `bash .agents/hooks/[name].sh` — this works on Windows because
  Copilot invokes commands through a shell that can find `bash.exe`
...
Copilot works with any editor, but the template is optimized for:

- **VS Code** with the Copilot extension
- Terminal-based Copilot CLI
