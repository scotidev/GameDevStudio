...
{
  "permissions": {
    "allow": ["Bash(git *)", "Bash(npm *)", "Read", "Glob", "Grep"],
...
Copilot supports different permission modes. Recommended for game dev:

### During Development (Default)

Use **normal mode** — Copilot asks before running most commands. This is safest
for production code.
...
Use **read-only** permissions — Copilot can read and search but not modify files.
